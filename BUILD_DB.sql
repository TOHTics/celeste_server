...
USE `celestesensordata`$$
CREATE DEFINER = CURRENT_USER TRIGGER `celestesensordata`.`Device_AFTER_DELETE` AFTER DELETE ON `Device` FOR EACH ROW
BEGIN
delete from APIUser where APIUser.id = old.id;
END$$
